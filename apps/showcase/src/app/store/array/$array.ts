...
export class $Array<T = any> implements Mergable<T[]> {

    private operations: OperationFn<T>[] = [];

    public static push<T>(...items: T[]): $Array<T> {
        return new $Array<T>().push(...items);
    }
     ...
     */
    public filter<S extends T>(predicate: (value: T, index: number, array: T[]) => value is S, thisArg?: any): this;
     ...
     * @param thisArg An object to which the this keyword can refer in the predicate function. If thisArg is omitted, undefined is used as the this value.
     */
    public filter(predicate: (value: T, index: number, array: T[]) => unknown, thisArg?: any): this;
